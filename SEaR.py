...
class fit:
    ...
    def setupMCMC(self):
        ...
        mean, st, vals = sigma_clip.meanclip(imagedata[max([self.impsfcenter[1]-50.,0]):min([self.impsfcenter[1]+50,imagedata.shape[0]-1]),
                                             max([self.impsfcenter[0] - 50., 0]):min([self.impsfcenter[0] + 50,imagedata.shape[1] - 1])],
                                             clipsig=4, maxiter=8)
        ...
        if self.ty + (self.stampsize - 1) / 2 > templatedata.shape[0]:
            raise Exception('candidate is too close to edge of ccd')
        else:
        ...
        if self.tx + (self.stampsize - 1) / 2 > templatedata.shape[1]:
            raise Exception('candidate is too close to edge of ccd')
        else:
        ...
        #raw_input()
        mean, st, vals = sigma_clip.meanclip(templatedata[max([self.templatepsfcenter[1]-50.,0]):min([self.templatepsfcenter[1]+50, templatedata.shape[0]-1]),
                                             max([self.templatepsfcenter[0] - 50., 0]):min([self.templatepsfcenter[0] + 50, templatedata.shape[1] - 1])],
                                             clipsig=4, maxiter=8)
    ...
    def runDMC(self):
        ...
        aaa = mcmc.metropolis_hastings(
              galmodel=     self.data[1,:,:]/10.#setting the initial guess of the galaxy/background model to the template image
            , modelvec=     np.array([self.initialguess,0])
            , galstd=       np.sqrt(np.abs(self.data[1,:,:]))/20. + 20.
