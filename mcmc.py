        ...
        if self.fix_gal_model:
            ...
            if flags == 0:
                if fitflags == 0.:
                    ...
                    [X, Y] = np.meshgrid(np.arange(20)/10.,np.arange(20)/10.)
                    ...
                    # fr3 = 10000. * np.exp(1j*(self.ff*(self.x_pix_offset)+self.ff*(self.y_pix_offset)))
                    # m, n = fr.shape
                    # gc = np.real(ifft2(S*fr2))
                    # gc = np.roll(gc, -m / 2 + 1, axis=0)
                    # gc = np.roll(gc, -n / 2 + 1, axis=1)

                    delta = np.fft.fftn(kicked_modelvec * S * fr2 * fr).real
