...
def run(listindex,index,root,templatedir,dontskip):
    ...
    for i,bc,x,y,sn,m in zip(range(detections['x'].shape[0]),detections['band_ccd'],detections['x'],detections['y'],detections['sn'],detections['mag']):
        ...
        if not os.path.exists('/project/projectdirs/des/p9smp/results31chichi/'):
            os.mkdir('/project/projectdirs/des/p9smp/results31chichi/')

        sd = '/project/projectdirs/des/p9smp/results31chichi/detections_'+detectionslist[listindex].split('/')[0]+'_' + tband + '_' + ccd + '.txt'
        #print 'outfile',sd
